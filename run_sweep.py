...
def plot_metrics(records, task_name, norms, out_dir: Path):
    ...
    # Unique comments: three lines by community split, averaged across all norms
    ax = axes_flat[len(metrics_to_plot)]
    for label, metric, color in [
        ('Both communities', 'unique_comments',   palette[3]),
        ('Community A',      'unique_comments_a', palette[0]),
...
def plot_community_metrics(records, task_name, norms, out_dir: Path):
    ...
    fig.suptitle(f"Community Pairs — {task_name}  |  norms = {norms}",
                 fontsize=15, fontweight="bold", y=1.02)
    fig.tight_layout()
...
def main():
    ...
    # plot_community_metrics(all_records, task_name, norms, out_dir)
